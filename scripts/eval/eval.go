...
const (
	COM_TLD_SPECS  = "http://localhost:3000/json/com_tldSpec.json"
	EXAMPLE_SPECS  = "http://localhost:3000/json/exampleSpec.json"
	WHATEVER_SPECS = "http://localhost:3000/json/whateverSpec.json"
)
...
func setupDNSInfo() {
	...
	cmd := exec.Command("./copy_specs.sh")
	cmd.Dir = "../json_server"
	copySpecsOut, _ := cmd.Output()
	fmt.Printf("Copied generated network specs: %s\n", string(copySpecsOut))
...
func runTests() {
	var (
		REQUESTS_PER_SECOND = [...]int{10, 100, 200, 300, 500, 1000}
	...
	for _, rps := range REQUESTS_PER_SECOND {
		testName := fmt.Sprintf("%dtld_%dtarg_%dv_%dn_%drps",
			...
			rps)
		benchmark.LaunchBenchmark(TOTAL_REQUESTS, rps, fmt.Sprintf("%s.csv", testName))
	}
}
